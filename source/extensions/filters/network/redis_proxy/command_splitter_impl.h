...
/**
 * ScanRequest sends the SCAN command to all Redis servers. The response from each Redis
 * (which must be a 2-element array [cursor, keys]) is merged and returned to the user.
 * The cursor is set to "0" when all shards return "0", indicating the scan is complete.
 */
...
/**
 * temporary ScanKeysRequest is a specialized version of ScanRequest that executes a keys command on redis,
 * returns responses back in scan contract with a hardcoded cursor of "0" to indicate completion.
 */
class ScanKeysRequest : public FragmentedRequest {
public:
  static SplitRequestPtr create(Router& router, Common::Redis::RespValuePtr&& incoming_request,
                                SplitCallbacks& callbacks, CommandStats& command_stats,
                                TimeSource& time_source, bool delay_command_latency,
...
private:
  ScanKeysRequest(SplitCallbacks& callbacks, CommandStats& command_stats, TimeSource& time_source,
              bool delay_command_latency)
      : FragmentedRequest(callbacks, command_stats, time_source, delay_command_latency) {}
  // RedisProxy::CommandSplitter::FragmentedRequest
  void onChildResponse(Common::Redis::RespValuePtr&& value, uint32_t index) override;
